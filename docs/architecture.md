...
| warp、CTA、Device 完成状态 | 各 lifecycle scope | `FROZEN（T5/01-core-warp-manager 子范围）`: Core 将本生命周期曾参与且 canonical mask 归零的 slot 标为 finished，并只聚合这些 slot；CTA/Device completion 仍为 `PROVISIONAL` | TMC mask=0 可退休 warp；从未激活的空 slot 不参与 Core completion。CTA/host completion 协议见 U-CTA-01/U-ABI-01，上层聚合结果不能反写低层完成真值。 |
...
`FROZEN（T5/01-core-warp-manager，lifecycle/completion）`：Core lifecycle 明确区分从未参与的 inactive、canonical running 且可选的 runnable、canonical running 但有非零 typed `BlockReason` 的 blocked，以及曾参与后 canonical inactive/zero-mask 的 finished。显式 `Block`/`Resume` 只改变 scheduler eligibility；`Activate` 只接纳调用者已经显式初始化为 running/nonzero mask 的 inactive owner，不补 PC/register/CSR/CTA ABI 默认值。`Warp.Step` 返回 deferred 或 fault 时该 warp 分别以派生 future-owner reason 或 fault reason阻塞，避免同一无进展 PC 被重复选择；TMC/PRED 零 mask经既有 canonical completion路径提交后，该 slot在同一个 Core step 返回边界成为 finished。Core completion只聚合本生命周期 `participated` slots，从未激活的空 slot不属于待完成集合；finished slot退出调度不会影响其他 runnable slot。
...
以下完整顺序仍是功能边界而不是 RTL cycle/pipeline 模型。四 lane普通及Warp-local SIMT执行的步骤 3–8与预算化步骤9子集已由第 5.11–5.16 节冻结；T5/01 已冻结固定四 slot 的步骤2选择以及 Warp finished 到 Core completion 的步骤9子范围，T5/02冻结WSPAWN/WSYNC在步骤7–9的Core协调子范围，T5/03冻结有限Core Run/trace和Multi-Warp completion。launch、BAR/CTA/Kernel/host completion仍是 `PROVISIONAL`：
...
9. **Progress/completion**：Warp manager 重新派生 runnable view；canonical zero-mask completion立即使该 participated slot退出调度，Core只聚合本生命周期相关 slots。后续 CTA、Device/kernel completion仍单向向上聚合；Harness 只经公共结果接口读取最终状态和内存。

## 7. 后续公共接口清单

除第 5.1、5.3、5.4、5.5、5.6 节已冻结的 ISA decoder/evaluator/effect 具体接口、第 5.8 节的 Lane/Warp State/View、第 5.9 节的本地原子 apply/forwarding、第 5.10 节的 caller-supplied-word 单指令连接、第 5.11–5.16 节明确冻结的四 lane普通与Warp-local SIMT边界，以及第 5.17–5.19 节冻结的 Core 四槽位/lifecycle/scheduler、WSPAWN/WSYNC协调、有限Run/trace/completion子集外，下表其余上层集成用途仍是 `PROVISIONAL`。箭头方向均从调用者到服务，结果/effect 显式返回；任何接口都不授权全局查找或旁路 mutation。
...
| Warp executor/manager | 四个既有 executor/owner refs、typed lifecycle/block、单步 context、WSPAWN stage、WSYNC pending view | round-robin selected warp、既有 Step result、slot lifecycle、atomic spawn、sync wait/retry、Core completion | `FROZEN（T5/01–02 子范围）`: Core → Warp/State coordinator → decoder/evaluator/services；每步只调用一个 Warp，低层不能反向选择 Core/CTA。CTA assignment与BAR仍待后续。 |
...
| Core Run/trace | finite step budget、逐step context、optional sink | complete/budget/blocked/deferred/trap/fault、attempt/retired/Last、detached scheduler records | `FROZEN（T5/03）`: caller → Core.Run → Core.Step；预算不代表cycle，trace不持有owner alias，Kernel/host completion仍不由此接口推断。 |
...
| U-WSYNC-01 | WSYNC pending provider最终如何组合异步owners，并保证何种 memory visibility/order？ | T1 已冻结显式pending-work view与wait/drain/release effect；T5/02已接入可注入provider/逐Step view并闭合block/retry/PC提交。 | pipeline pending集合到异步memory/barrier owner组合及功能级ordering的最小映射、software convention缺失。 | Warp executor/lifecycle、Memory ordering、Barrier integration。 | 多warp memory/barrier集成前。 |
...
| U-MEM-01 | global/LMEM 的最终地址范围与 scope、FENCE/BAR/WSYNC 的跨 warp 可见性、同指令重叠lane store优先级及 self-modifying code 规则是什么？ | Integer LSU 已冻结自然对齐、typed request/response、可选 bounds view、四lane request coverage、atomic batch owner和 FENCE ordering 边界；RTL 仍按 LMEM base/size 分 local/global lane masks并走不同 physical paths。 | cache/LSU wiring 不等于跨 scope/重叠写可见性 contract；软件/checker约定缺失。当前flat owner原子拒绝重叠batch而不猜lane顺序。 | Instruction source、Memory spaces、Barrier/ordering owners。 | 最终 Memory scope/ordering 集成前。 |
...
| 3. ISA decode/evaluate 边界 | 第 5.1、5.7、5.10–5.19 节 | ISA 只读最小 view、产生显式 effect，不持有长期状态或调度上层；最终自动门禁要求 105 条 catalog/decode/evaluator/vector 一一对应，T2–T5 连接证明真实 State 只经该边界调用 ISA。 |
...
| 6. 后续公共接口职责 | 第 7 节 | decoder/evaluator/effect 已由T1冻结，T2已冻结State/View/apply，T3已冻结fetch/memory Step/bounded Run/trace，T4已把普通执行、同步memory与Warp-local SIMT扩展到四lane active mask，T5已冻结四-slot Core lifecycle/scheduler、WSPAWN/WSYNC、Run/trace/completion；Barrier/CTA/Kernel及最终memory scopes仍保持`PROVISIONAL`。 |
...
- 原始 T0 基线交付仅修改本文件；后续 T1 已新增无长期状态的ISA decoder/evaluators，T2已新增canonical State/View、原子effect stage/future-owner forwarding及caller-supplied-word连接。T3新增了不持有第二份状态/bytes的single-lane fetch、同步memory Step、跨owner coordinated commit、bounded Run与detached trace；T4/01加入普通四lane active-mask执行与原子multi-address store batch，T4/02加入Warp-local mask/divergence/cross-lane custom，T4/03闭合hand-built完整SIMT stream及显式起止SIMT trace；T5/01新增严格四-slot Core references、typed scheduler lifecycle、functional round-robin与Core completion子范围，T5/02新增WSPAWN source/targets原子协调与WSYNC显式pending block/retry，T5/03新增有限Core Run、outcome/completion聚合、detached trace与Multi-Warp端到端程序。CTA/Barrier coordinator、最终global/LMEM architecture、Kernel/host completion及cache/MMU/scoreboard/pipeline/cycle/timing/performance model仍未新增。外部`Vortex_rtl`始终作为只读输入，不属于交付内容。
