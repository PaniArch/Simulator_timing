...
以下完整顺序仍是功能边界而不是 RTL cycle/pipeline 模型。四 lane普通及Warp-local SIMT执行的步骤 3–8与预算化步骤9子集已由第 5.11–5.16 节冻结；launch、runnable warp选择和CTA/Kernel/host completion仍是 `PROVISIONAL`：
...
除第 5.1、5.3、5.4、5.5、5.6 节已冻结的 ISA decoder/evaluator/effect 具体接口、第 5.8 节的 Lane/Warp State/View、第 5.9 节的本地原子 apply/forwarding、第 5.10 节的 caller-supplied-word 单指令连接，以及第 5.11–5.16 节明确冻结的四 lane普通与Warp-local SIMT `InstructionSource`/`MemoryService`/`AtomicMemoryService`/`Warp.Step`/bounded `Run`/trace子集外，下表其余上层集成用途仍是 `PROVISIONAL`。箭头方向均从调用者到服务，结果/effect 显式返回；任何接口都不授权全局查找或旁路 mutation。
...
| 3. ISA decode/evaluate 边界 | 第 5.1、5.7、5.10–5.16 节 | ISA 只读最小 view、产生显式 effect，不持有长期状态或调度上层；最终自动门禁要求 105 条 catalog/decode/evaluator/vector 一一对应，T2/T3/T4 连接证明真实 State 只经该边界调用 ISA。 |
...
| 6. 后续公共接口职责 | 第 7 节 | decoder/evaluator/effect 已由T1冻结，T2已冻结State/View/apply，T3已冻结fetch/memory Step/bounded Run/trace，T4已把普通执行、同步memory与Warp-local SIMT扩展到四lane active mask；scheduler、Barrier/CTA/Kernel及最终memory scopes仍保持`PROVISIONAL`。 |
