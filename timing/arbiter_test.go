...
func TestArbiterIRAndUnknowns(t *testing.T) {
	for _, tc := range []struct {
		id, policy string
		inputs     int
	}{{"b-alu-merge", "R", 2}, {"b-alu-muldiv-merge", "P", 2}, {"b-sfu-merge", "R", 2}, {"b-fpu-backend-out", "R", 4}} {
		a, err := Arbiter(tc.id)
	...
	for _, tc := range []struct{ old, new string }{{"inputs: 2", "inputs: null"}, {"inputs: 2", "inputs: 0"}, {"policy: R", "policy: unknown"}, {"sticky: 0", "sticky: 1"}, {"model: 1", "model: 2"}, {"model: 1", "model: null"}} {
		if _, err := arbiter([]byte(strings.Replace(fixture, tc.old, tc.new, 1)), "b"); err == nil {
			t.Fatal("accepted unknown profile", tc)
		}
	}
	if _, err := Arbiter("b-schedule"); err == nil {
