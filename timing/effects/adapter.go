...
func (a *Adapter) Begin(token model.Token) error {
	...
	if token.Epoch != a.epoch || token.ID <= a.lastID || token.Warp != snapshot.WarpID() || token.PC != snapshot.PC() || isa.LaneMask(token.Mask) != snapshot.ActiveMask() {
...
func (a *Adapter) Observe(cycle uint64, r model.CoreReport, context state.ReadContext) (err error) {
	if a.failed {
		return fmt.Errorf("effect adapter requires reset")
	}
	defer func() {
	...
	if i.packed != nil {
		return a.observePacked(cycle, r, context)
	}
	snapshot, err := a.owner.Snapshot()
	if err != nil {
		return err
	...
	if r.Read.Valid {
		if i.capture == nil {
			i.capture, err = state.NewOperandCapture(snapshot, i.token.Word, context)
			if err != nil {
				return err
			}
		}
	...
	for class, s := range r.Executed {
		...
		i.delivery, err = a.owner.NewEffectDelivery(e)
		if err != nil {
			return err
		}
