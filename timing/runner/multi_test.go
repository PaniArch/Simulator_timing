...
func TestMultiRunnerConcurrentFunctionalResults(t *testing.T) {
	refs, refRAM := multiSetup(t)
	for _, owner := range refs {
		functional, err := warp.NewWithMemory(owner, refRAM)
		if err != nil {
			t.Fatal(err)
		}
		for n := 0; n < 8; n++ {
	...
	for trial := 0; trial < 3; trial++ {
		...
		options := runner.MultiOptions{Options: runner.Options{Backend: "std", PeriodPS: 1, FetchCycles: 2, MemoryCycles: 60}}
		if trial == 2 {
			options.Ready = func(c uint64) bool { return c%4 != 0 }
			options.MemoryDelay = func(t model.Token) uint64 {
				if t.Warp == 0 {
					return 100
				}
				return 17
		...
		if err = r.Run(15, observe); err != nil {
			t.Fatal(err)
		}
