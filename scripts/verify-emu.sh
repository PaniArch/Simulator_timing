...
if packages="$(go list -mod=vendor "${emu_packages[@]}")"; then
    if [[ -n "${packages}" ]]; then
        echo "verify: go build -mod=vendor ./..."
        run_quiet go build -mod=vendor "${emu_packages[@]}" || fail "go build failed"

        echo "verify: go test -mod=vendor ./..."
        run_quiet go test -mod=vendor "${emu_packages[@]}" || fail "go test failed"

        echo "verify: go vet -mod=vendor ./..."
        run_quiet go vet -mod=vendor "${emu_packages[@]}" || fail "go vet failed"
    else
